...
function makeNonUINode(name: string): IHierarchyNode {
  return {
    gameObject: {
      id: '50000',
      typeId: '1',
...
describe('UnityHierarchyRenderer – layout', () => {
...
describe('UnityHierarchyRenderer – hierarchy tree', () => {
  it('creates tree entries with correct GameObject names', () => {
    ...
    const el = UnityHierarchyRenderer.render([root]);
    const panel = el.querySelector('.unity-hierarchy-panel')!;
...
describe('UnityHierarchyRenderer – visual rendering', () => {
  it('renders RectTransform nodes with a blue outline', () => {
    const node = makeRectNode('Panel');
    const el = UnityHierarchyRenderer.render([node]);
    const viewport = el.querySelector('.unity-render-viewport')!;
...
describe('UnityHierarchyRenderer – scriptGuidMap resolution', () => {
  ...
  it('resolves custom script GUIDs to class names from the map', () => {
    ...
    const el = UnityHierarchyRenderer.render([node], scriptMap);
    const panel = el.querySelector('.unity-hierarchy-panel')!;
  ...
  it('resolves multiple different custom scripts in a hierarchy', () => {
    ...
    const el = UnityHierarchyRenderer.render([root], scriptMap);
    const panel = el.querySelector('.unity-hierarchy-panel')!;
