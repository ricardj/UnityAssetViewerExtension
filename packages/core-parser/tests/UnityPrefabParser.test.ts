...
import {
  UnityYamlParser,
  UnityPrefabHierarchyBuilder,
  UnityPrefabModificationApplier,
  type IParsedPrefab,
  type IUnityObject,
  type IHierarchyNode,
} from '../src/index';
...
describe('UnityYamlParser', () => {
  it('parses the sample prefab into 6 objects', () => {
    const result = UnityYamlParser.parse(samplePrefab);
  ...
  it('returns an empty objects array for YAML with no Unity documents', () => {
    const result = UnityYamlParser.parse('%YAML 1.1\n%TAG !u! tag:unity3d.com,2011:\n');
...
`;
    ...
    const result = UnityYamlParser.parse(malformedYaml);
...
describe('UnityPrefabHierarchyBuilder', () => {
  it('produces a single root node named Canvas', () => {
    const { objects } = UnityYamlParser.parse(samplePrefab);
    const roots = UnityPrefabHierarchyBuilder.build(objects);
  ...
  it('returns empty array when given no objects', () => {
    const roots = UnityPrefabHierarchyBuilder.build([]);
    expect(roots).toEqual([]);
  });

  it('handles Transform pointing to a nonexistent m_Father ID', () => {
    const objects: IUnityObject[] = [
      {
        id: '1',
        typeId: '1',
        typeStr: 'GameObject',
...
describe('UnityPrefabModificationApplier', () => {
  function makeBaseParsed(): IParsedPrefab {
    return {
      objects: [
        {
          id: '100',
  ...
  it('applies a simple property modification', () => {
    ...
    const result = UnityPrefabModificationApplier.apply(base, variant);
  ...
  it('does not mutate the original base parsed object', () => {
    ...
    UnityPrefabModificationApplier.apply(base, variant);
